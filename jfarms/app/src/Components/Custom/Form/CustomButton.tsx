 ...
 * @param loading indicator for when the button is in a loading state. Button is disabled if true
 * @param loadingIcon Icon to indicate loading state. Works best with svgs with no height and width styles set
 *
 */
export default function CustomButton({
  ...
  const _loadingStyle = loading ? " cursor-progress " : "";
  ...
  const _loadingIcon =
    loadingIcon === undefined
      ? createIcon(_defaultLoadingIcon)
      : createIcon(loadingIcon);
  ...
  return (
    <div className="relative px-0">
      ...
      >
        {loading ? _loadingIcon : _startIcon}
        {rest.children}
        {_endIcon}
      </button>
